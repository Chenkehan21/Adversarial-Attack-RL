...
class Agent(object):
    ...
    # set env:
    # 1. set body/joint(每个Anget都是由body和joint组成的)
    #    就是通过导入的xml文件来设置agent属性，包括位置，自由度，速度等信息
    #    作为一个固定的属性，实例化时候运行一次
    # 2. set observation_space / action_space
    #    就是通过导入的xml文件来设置observation_space / action_space
    #    作为一个固定的属性，实例化时候运行一次
    ...
    # 定义observation space box(high, low)
    def set_action_space(self):
        ...
        for i, motor in enumerate(list(acts)):
            ...
            if ctrl:
                ...
                high[i] = chigh
                low[i] = clow
                # default 的 motor 和这里的 motor 有什么不同？
        self._low = low
        self._high = high
        self.action_space = Box(low, high)
    # 定义action space box(high, low)
    ...
    #   属性：body
    #   self.body_ids       数组          在xml的id，通常是连续的
    #   self.body_dofnum    数组          自由度
    #   self.nv             值            自由度总数
    #   self.body_dofadr    数组          单一自由度data的地址位置
    #   self.qvel_start_idx 值            bodies自由度的(文件中)起始地址
    #   self.qvel_end_idx   值            bodies自由度的(文件中)结束地址
    ...
    #   属性：
    #   self.joint_ids      数组          在xml的joint id，通常是连续的
    #   self.jnt_nqpos      数组          单一position维数
    #   self.np             值            positions维数
    #   self.jnt_qposadr    数组          单一position data的地址位置
    #   self.qpos_start_idx 值            joint位置信息的(文件中)起始地址
    #   self.qpos_end_idx   值            joint位置信息的(文件中)结束地址
