...
class KickAndDefendV2(MultiAgentEnv):
    def __init__(self, max_episode_steps=500, randomize_ball=True, **kwargs):
        super(KickAndDefendV2, self).__init__(**kwargs)
        ...
        self.LIM_X = [(-3.5, -0.5), (1.6, 3.5), (1.6, 3.5)]
        # self.RANGE_X = self.LIM_X = [(-2, -2), (1.6, 3.5)]
        self.LIM_Y = [(-2, 2), (-2, 0), (0, 2)]
        self.RANGE_X = self.LIM_X.copy()  # reset 用
        self.RANGE_Y = self.LIM_Y.copy()  # reset 用
        self.BALL_LIM_X = (-2, 1)
        self.BALL_LIM_Y = (-4, 4)
        self.BALL_RANGE_X = self.BALL_LIM_X  # random ball reset 用
        self.BALL_RANGE_Y = self.BALL_LIM_Y  # random ball reset 用
