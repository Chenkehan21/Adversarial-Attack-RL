...
DEFAULT_ENV_NAME = "PongNoFrameskip-v0"
FPS = 25


# frame of second
# buffer 4 * 84 * 84
def FGSM_ATTACK(state, net, epislon=0.001, device=torch.device("cuda")):
    state = state.to(device)
    state.requires_grad = True
    output = net(state)
    output = nn.Softmax(dim=1)(output)
    target = torch.tensor([5], dtype=torch.long).to(device)
    Iter_no = 0
    Iter_max_no = 5
...
if __name__ == "__main__":
    ...
    state = torch.load(".\PongNoFrameskip-v4-best_-13.dat", map_location=lambda stg, _: stg)
